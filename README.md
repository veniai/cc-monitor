...
**自动恢复** — API 报错自动重试；检测到配额限额后自动解析终端显示的重置时间，到点恢复，无需人工干预（已验证智谱 GLM 包月套餐 5 小时限额，其他提供商待验证）
...
**权限自动批准** — 所有权限请求自动批准并通知用户；AskUserQuestion 在龙虾模式下等待用户通过 IM 回复，不自动选择
...
|------|------|
| **Stop** | 任务正常完成 → 发送完成通知 |
| **StopFailure** | API 报错 → 自动恢复；配额限额 → 解析终端重置时间，暂停重试，到时间自动恢复（已验证智谱 GLM，其他待验证） |
| **PermissionRequest** | 全部自动批准 + 通知用户；AskUserQuestion（龙虾模式）等待用户通过 IM 回复 |
