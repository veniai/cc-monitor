...
**自动恢复** — API 报错自动重试；检测到配额限额后解析重置时间，到点自动恢复，无需人工干预（适配 GLM 智谱 5 小时套餐）
...
**权限自动批准** — 所有权限请求自动批准并通知用户；AskUserQuestion 等待用户通过 IM 回复，不自动选择
...
| **Stop** | 任务正常完成 → 发送完成通知 |
<<<<<<< HEAD
| **StopFailure** | API 报错 → 自动恢复；配额限额（适配 GLM 智谱 5 小时套餐）→ 解析重置时间，暂停重试，到时间自动恢复 |
| **PermissionRequest** | 全部自动批准 + 通知用户；AskUserQuestion 等待用户通过 IM 回复 |
