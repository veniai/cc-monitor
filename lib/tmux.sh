...
recover_session() {
  ...
  local message="${2:-$(config_get "monitor:recovery_message" "临时中断。恢复后读 TaskList 继续当前 task：有 skill 必须调 Skill tool 并严格按 skill 流程执行（不得 inline 替代）；单步过长可拆分但不跳过；已完成不重做")}"
